...
/**
 * PlayerBuildingProductionSearch represents the model behind the search form of `app\models\PlayerBuildingProduction`.
 */
class PlayerBuildingProductionSearch extends PlayerBuildingProduction
{
    ...
    {
        return [
            [['id', 'player_building_id', 'item_id'], 'integer'],
            [['end_time', 'created_at', 'updated_at', 'created_at_range', 'updated_at_range', 'end_time_range'], 'safe'],
        ];
    }
    ...
    {
        $query = PlayerBuildingProduction::find();
        ...
        if (!empty($this->end_time_range)) {
            ...
            $query->andFilterWhere(['between', 'end_time', $start, $end]);
        }
